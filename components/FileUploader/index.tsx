  ...
  return (
    <>
      ...
      >
        ...
        {
          (buttonType === 'button' && !value) ? 'בחירת קובץ' : (
            <Flex gap='22px' direction='column' alignItems='center'
                  justifyContent='center'>
              {
                value ? <Box width='75px' height='56px'/> :
                  <Image src={ImagePlaceholder} width={75} height={56} alt=''/>
              }
              <Flex gap='9px' justifyContent='center' alignItems='center'
                    height='fit-content'>
                <p>{value ? 'החלף קובץ' : 'בחירת קובץ'}</p>
                {
                  value ? <Image src={ReplaceIcon} width={20}
                                 height={17} alt=''/> :
                    <Image src={UploadingIcon} width={15}
                           height={16} alt=''/>
                }
      ...
      <input
        ref={inputRef}
        onChange={(e) => onChange && e.target.files && onChange(e.target.files[0])}
        hidden
